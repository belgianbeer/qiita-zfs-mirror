...
[^perf]: ZFSはCopy on Writeでデータ更新を行なう関係で、空き容量が少なくなると他のファイルシステムより極端にパフォーマンスが低下する傾向があるため(特にハードディスクで顕著)、容量ギリギリまで利用するのは得策ではありません。どのぐらいまで使うとどうパフォーマンスが低下するのかに関してはワークセットとの兼ね合いなので単純に示すことはできないですが、他のファイルシステムなら影響の無い空き容量であってもZFSだと影響が出ることがあるので注意するべきと覚えておきましょう。
...
筆者の場合は後者の1台づつ交換する方法を取りましたが、この方式の最大のメリットは、パフォーマンスの低下はあるものの同期中も通常通りサーバーを使い続けられることでしょう。

## ハードディスクの交換
### ハードディスク交換時の注意

ZFSのミラーでハードディスクを交換する場合に注意することは、ディスクの交換前にZFSプールの操作、例えば `zpool detach zvol0 gpt/ndisk2` などを **実行しないことが望ましいです**。なぜなら`zpool detach`でndisk2をプールから外すとndisk2内のデータは消去された扱いになり、万が一新しいハードディスクとのデータの同期中にndisk1の読み出しでエラーが発生した場合にndisk2に交換して同期をやり直すことができなくなってしまいます[^undestroy]。電源を切ってndisk2を外すだけであればオフラインであってもndisk2内のデータはndisk1と同じままなので、万が一のときにndisk2を使ってやり直しができます。
...
通常であれば`zpool replace`の元になるデバイスにはRAID等を構成しているデバイス名を指定するのですが、今回は交換のために取り外しているため、すでに存在しません。このような場合は`zpool status zvol0`で表示されているIDを指定します。
...
データ量は1.5TB程度あるため同期にはそれなりの時間がかかるのですが、バックグラウンドで行われるのでなんのメッセージも表示されません。この間状態がわからないと少しばかり不安になるのですが、ZFSでは同期の進行状況を`zpool status`で確認できます。
```
$ zpool status zvol0
  pool: zvol0
 state: DEGRADED
